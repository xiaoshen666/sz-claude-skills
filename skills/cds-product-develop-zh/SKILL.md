...
**可选择执行的开发方案**（用户可选择一个或多个）：

| 选项 | 包含步骤 | 名称 | 详细指南 | 核心交付物 |
|------|---------|------|---------|-----------|
| **A** | 步骤1+3 | 前端开发 | references/FRONTEND-MODULE-STRUCT-RULES.md + FRONTEND-IMPL.md | 前端项目结构、前端组件代码、前端服务代码、前端路由配置 |
| **B** | 步骤2+4+5 | 后端开发 | references/BACKEND-MODULE-STRUCT-RULES.md + BACKEND-IMPL.md + STARTUP-CODE.md | 后端项目结构、后端控制器代码、后端服务代码、后端数据访问代码、启动类代码（按需） |
...
| 步骤2 | 后端项目结构创建 | 创建后端目录结构、包结构 | 选项B |
| 步骤3 | 前端代码生成 | 生成前端组件、服务、路由代码 | 选项A |
| 步骤4 | 后端代码生成 | 生成后端Controller、Service、DAO代码 | 选项B |
| 步骤5 | 启动类代码生成 | 按需生成启动类、配置文件、依赖注入配置 | 选项B |
| 步骤6 | 模块集成测试 | 前后端集成测试、功能验证 | 选项C |

**选项B中的启动类代码生成（步骤5）支持细分选择**：
- 应用启动类
- 配置文件
- 依赖注入配置

用户在选择选项B后，需要对上述三项进行单独确认，可以选择任意一项、多项或全部不选。
...
- [ ] **选项A：前端开发**（包含前端项目结构创建 + 前端代码生成）
- [ ] **选项B：后端开发**（包含后端项目结构创建 + 后端代码生成 + 启动类代码生成）
- [ ] **选项C：模块集成测试**
...
**选择规则**：
1. 用户可以选择任意一个或多个选项
2. 如果选择选项B，需要在执行步骤5时对启动类代码生成的三项内容进行细分确认
3. 选项之间处于同一等级，无强制先后顺序
4. 各选项内部步骤按顺序执行（如选项A：先执行步骤1，再执行步骤3）
