...
1. **设计阶段**：把页面需求结构化（需求概要确认、需求详细设计、数据架构设计、业务流程设计、UI稿生成、前端架构设计、后端架构设计、设计验收检查）
2. **开发阶段**：把结构化设计实现成代码（模块开发初始化、前端开发、后端开发、模块集成测试）
3. **测试阶段**：把代码和设计进行独立验证（设计阶段检查验收、开发阶段测试执行）
4. **运维阶段**：把验证通过的代码交付到环境中（运维环境准备、模块部署）
...
| 开发 | 前端代码 | 测试、运维 | 作为前端编译、启动、部署输入 |
| 开发 | 后端代码 | 测试、运维 | 作为后端编译、启动、部署输入 |
| 开发 | 启动类代码 | 测试、运维 | 作为应用启动入口 |
| 开发 | 模块集成产物 | 测试 | 作为模块集成测试输入 |
| 测试 | 全局设计检查报告 | 设计 | 作为设计产物改进依据 |
| 测试 | 模块设计检查报告 | 设计 | 作为模块设计改进依据 |
| 测试 | PRD测试用例检查报告 | 设计 | 作为测试用例完善依据 |
| 测试 | 原型检查报告 | 设计 | 作为原型优化依据 |
| 测试 | 模块测试报告 | 开发 | 作为模块代码修复依据 |
| 测试 | 集成测试报告 | 开发 | 作为集成问题修复依据 |
| 测试 | 验收报告 | 运维 | 作为上线部署依据 |
...
1. 使用 `cds-product-design-zh` 输出完整设计产物（需求详细设计、UI稿、前后端架构设计、API设计文档）
2. 使用 `cds-product-test-zh` 做设计检查或方案验收
3. 使用 `cds-product-develop-zh` 基于设计产物生成代码（前端开发、后端开发、模块集成测试）
4. 再次使用 `cds-product-test-zh` 做模块测试、集成测试与项目验收
